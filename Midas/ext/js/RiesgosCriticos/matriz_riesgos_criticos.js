...
function obtenerModalTecnologias() {
    ...
    $.ajax({
        url: "/evr/RiesgosCriticos/ObtenerTecnologias",
        //url: "../RiesgosCriticos/ObtenerTecnologias",
        type: "post",
...
$(document).ready(function () {
            ...
            success: function (datos) {
                if (datos != null) {
                    ...
                    if (datoimagen == "") {
                        ...
                        $('#dialog').empty();
                        $('#dialog').append('<div style="text-align: center;" class="center"><img style="width: 100%; height: 100%;" method="post" enctype="multipart/form-data" src="../' + datos + '"/><div>').append($(this).html());
    ...
    function getDatos() {
        ...
        $.ajax({
            url: "/evr/RiesgosCriticos/ObtenerActivosCriticos",
            //url: "../RiesgosCriticos/ObtenerActivosCriticos",
            type: "post",
        ...
        $('#datatableMatrizRiesgos').find('tr').each(function () {
            ...
            $(tr).find('td').each(function () {
                ...
                if (indice > 0 && indice < 9) {
                    //riesgo = parseInt($('#datatableMatrizRiesgos').find('th')[indice].innerText); //este caso es para cuando en el innerText exista un numero y no una cadena
                    var riesgoObjeto = $('#datatableMatrizRiesgos').find('th')[indice];
                    riesgo = parseInt(riesgoObjeto.getAttribute("tiporiesgocritico"));
                }
                    ...
                    $.each(arrayActivos, function (i, arrayActivos) {
                        if (filaObjeto.tecnologia == arrayActivos.id_tecnologia) {
                            ...
                            if (tr[0].classList.contains("AREA")) {
                                if (filaObjeto.area == arrayActivos.id_areanivel1 && arrayActivos.id_riesgoCritico == riesgo && arrayActivos.activo == true) {
                                    chk.prop('checked', 'true');
                                }
                            }
                            if (tr[0].classList.contains("SISTEMA")) {
                                if (filaObjeto.sistema == arrayActivos.id_areanivel2 && arrayActivos.id_riesgoCritico == riesgo && arrayActivos.activo == true) {
                                    chk.prop('checked', 'true');
                                }
                            }
                            ...
                            if (tr[0].classList.contains("EQUIPO")) {
                                if (filaObjeto.equipo == arrayActivos.id_areanivel3 && arrayActivos.id_riesgoCritico == riesgo && arrayActivos.activo == true) {
                                    chk.prop('checked', 'true');
                                }
                            ...
                            if (tr[0].classList.contains("NIVELCUATRO")) {
                                if (filaObjeto.nivelcuatro == arrayActivos.id_areanivel4 && arrayActivos.id_riesgoCritico == riesgo && arrayActivos.activo == true) {
                                    chk.prop('checked', 'true');
                                }
                                // && filaObjeto.sistema == arrayActivos.id_areanivel2 && filaObjeto.area == arrayActivos.id_areanivel1 &&
                            }
    ...
    function EditarRegi(matrizRiesgos, i, nombre, tecnologia, version, tipo) {
        ...
        $('.loader').show();
        $.ajax({
            //url: "../RiesgosCriticos/ActualizarRegistro", //Your path should be here
            url: "/evr/RiesgosCriticos/ActualizarRegistro", //Your path should be here
            ...
            success: function (datos) {
                if (datos != null) {
                    CancelarEdicion();
                    getDatos();
                    MostrarMensaje("Fila Guardada", "success",800)
                    // location.reload();
                    if (datos != "") {
        ...
        if (comprobarEdicion()) {
            ...
            $(filanueva).find('td').each(function () {
                if ($(this)[0].hasAttribute('ColumnaPrincipal')) {
                    ...
                    textofinal = texto;
                    $(this).empty().append('<input name="nuevaFila"  type="text" value="' + texto + '" id="editarNombre" class="form-control" disabled></td>');
                }
